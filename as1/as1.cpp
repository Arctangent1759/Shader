...
void drawHeart(double centerX, double centerY, double dimension, vector<double> ca, vector<double> cr, vector<double> cs, double p, vector<Light*> lights){
  ...
  glLoadIdentity();
  glBegin(GL_POINTS);
  png::image<png::rgb_pixel_16> heart(viewport.w, viewport.h);

  int minI = max(0,(int)floor(centerX-4.0*dimension))-6;
  int maxI = min(viewport.w-1,(int)ceil(centerX+4.0*dimension))+6;
  int minJ = max(0,(int)floor(centerY-3.0*dimension))-6;
  int maxJ = 2.0*min(viewport.h-1,(int)ceil(centerY+3.0*dimension))+6;

  vector< vector< vector<double> > > geometry(maxI-minI);
  for (int i = 0; i < geometry.size(); i++){geometry[i]=vector<vector <double> >(maxJ-minJ);}

  for (int j=minJ;j<maxJ;j++) {
    for (int i=minI;i<maxI;i++) {
      double x = (i+0.5-centerX);
      double y = (j+0.5-centerY);
      if (pow(abs(x/(4.0*dimension)),2)+pow(y/(3.0*dimension)-pow(abs(x/(4.0*dimension)),2.0/3.0),2)<=1.0){
        //Get z
        double y1=-dimension*(3.0/4.0)*(sqrt(16.0-pow(abs(x)/dimension,2.0))-pow(2.0*abs(x)/dimension,2.0/3.0));
        double y2=dimension*(3.0/4.0)*(sqrt(16.0-pow(abs(x)/dimension,2.0))+pow(2.0*abs(x)/dimension,2.0/3.0));
        double ym = (y1+y2)/2.0;
        double yd = abs(y1-y2)/2;
        double z=(4.0*dimension)*sqrt(1.0-pow(y-ym,2)/pow(abs(yd),2))*sqrt(1.0-pow(x,2)/pow(4.0*dimension,2));
        geometry[i-minI][j-minJ]=makeVector(x,y,z);
      }else{
        geometry[i-minI][j-minJ]=makeVector(NAN,NAN,NAN);
      }
    }
  }
  double gi, gj;
  for (int i=minI-1;i<maxI+1;i++) {
    for (int j=minJ+1;j<maxJ-1;j++) {
      ...
      if (pow(abs(x/(4.0*dimension)),2)+pow(y/(3.0*dimension)-pow(abs(x/(4.0*dimension)),2.0/3.0),2)<=1.0){
        gi=i-minI;
        gj=j-minJ;
        vector<double> pos = geometry[gi][gj] + makeVector(centerX,centerY,0);
        vector<double> e = makeVector(0,0,1);
        vector<double> n = normalize(cross(geometry[gi][gj+6]-geometry[gi][gj-6],geometry[gi-6][gj]-geometry[gi+6][gj]));
        vector<double> clr = mult(cr,ca);
        ...
        if (args.outFile == "") {
          ...
          heart[heart.get_height()-1-j][i]=png::rgb_pixel_16(max(0.0, min(1.0, clr[0]))*65535, max(0.0, min(1.0, clr[1]))*65535, max(0.0, min(1.0, clr[2]))*65535);
        }
      }
    }
  }
...
void drawSpheres(vector<Sphere> spheres, vector<double> ca, bool shadow){
  ...
  glLoadIdentity();
  glBegin(GL_POINTS);
  png::image<png::rgb_pixel_16> image(viewport.w, viewport.h);
      ...
      for (int j=0;j<viewport.h;j++) {
        ...
        if (distSq<=radius*radius) {
          ...
          if (args.outFile == "") {
            ...
            image[image.get_height()-1-j][i]=png::rgb_pixel_16(max(0.0, min(1.0, clr[0]))*65535, max(0.0, min(1.0, clr[1]))*65535, max(0.0, min(1.0, clr[2]))*65535);
          }
        }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++){
    ...
    }else if ((string)argv[i]=="--heart"){ 
      myDisplay = heartRender;
